  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.totalsBar}>
        <button
          className={`${styles.totalItem} ${activeDocType === 'amendements' ? styles.totalItemActive : ''}`}
          onClick={() => onTotalClick('amendements')}
        >
          <strong>{(docCounts?.amendements ?? 0).toLocaleString('fr-FR')}</strong> amendements
        </button>
        <span className={styles.totalSep}>·</span>
        <button
          className={`${styles.totalItem} ${activeDocType === 'questions' ? styles.totalItemActive : ''}`}
          onClick={() => onTotalClick('questions')}
        >
          <strong>{(docCounts?.questions ?? 0).toLocaleString('fr-FR')}</strong> questions écrites
        </button>
        <span className={styles.totalSep}>·</span>
        <button
          className={`${styles.totalItem} ${activeDocType === 'interventions' ? styles.totalItemActive : ''}`}
          onClick={() => onTotalClick('interventions')}
        >
          <strong>{(docCounts?.interventions ?? 0).toLocaleString('fr-FR')}</strong> interventions
        </button>
        <span className={styles.totalSep}>·</span>
        <button
          className={`${styles.totalItem} ${activeDocType === 'dossiers' ? styles.totalItemActive : ''}`}
          onClick={() => onTotalClick('dossiers')}
        >
          <strong>{(docCounts?.dossiers ?? 0).toLocaleString('fr-FR')}</strong> dossiers
        </button>
      </div>
